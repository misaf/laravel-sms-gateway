...
test('the timeout and retry config has its own defaults', function (): void {
    expect(config('sms-gateway-textlocal.timeout.server'))->toBe(5)
        ->and(config('sms-gateway-textlocal.timeout.client'))->toBe(6)
        ->and(config('sms-gateway-textlocal.retry.times'))->toBe(2)
        ->and(config('sms-gateway-textlocal.retry.sleep_milliseconds'))->toBe(100);
});

test('casts string timeout and retry values from the environment to integers', function (): void {
    $_SERVER['SMS_GATEWAY_TEXTLOCAL_SERVER_TIMEOUT'] = '15';
    $_SERVER['SMS_GATEWAY_TEXTLOCAL_CLIENT_TIMEOUT'] = '30';
    $_SERVER['SMS_GATEWAY_TEXTLOCAL_RETRY_TIMES'] = '4';
    $_SERVER['SMS_GATEWAY_TEXTLOCAL_RETRY_SLEEP_MILLISECONDS'] = '250';

    try {
        $config = require __DIR__ . '/../../config/sms-gateway-textlocal.php';
    } finally {
        unset(
            $_SERVER['SMS_GATEWAY_TEXTLOCAL_SERVER_TIMEOUT'],
            $_SERVER['SMS_GATEWAY_TEXTLOCAL_CLIENT_TIMEOUT'],
            $_SERVER['SMS_GATEWAY_TEXTLOCAL_RETRY_TIMES'],
            $_SERVER['SMS_GATEWAY_TEXTLOCAL_RETRY_SLEEP_MILLISECONDS'],
        );
    }
