...
when the gateway was never reached — with the driver name `smsir`. See the
...
- `api_key` — your SMS.ir API key (`SMS_GATEWAY_SMSIR_API_KEY`), sent as the `X-API-KEY` header; required — a missing or empty environment variable fails when the driver is resolved
- `base_url` — the endpoint (`SMS_GATEWAY_SMSIR_BASE_URL`), defaulting to `https://api.sms.ir/v1/`; required and may not be empty — it is the single source of truth for the endpoint, so point it at a proxy or a sandbox by editing it here
- `timeout.server` — the connection timeout in seconds (`SMS_GATEWAY_SMSIR_SERVER_TIMEOUT`), defaulting to `5`
- `timeout.client` — the request timeout in seconds (`SMS_GATEWAY_SMSIR_CLIENT_TIMEOUT`), defaulting to `6`; keep it above the connection timeout
- `retry.times` — how many attempts a send gets (`SMS_GATEWAY_SMSIR_RETRY_TIMES`), defaulting to `2`
- `retry.sleep_milliseconds` — the pause between attempts (`SMS_GATEWAY_SMSIR_RETRY_SLEEP_MILLISECONDS`), defaulting to `100`
