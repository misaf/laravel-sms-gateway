...
driver lives in `src/Drivers/laravel-sms-gateway-<driver>` and is split out to
...
- `src/SmsGatewayDriver.php` — the abstract HTTP driver all provider drivers extend.
- `src/SmsGatewayManager.php` — resolves drivers; ships only the `null` driver.
...
- `src/Drivers/<package>/` — a full composer package: `composer.json`, `config/`,
...
- A new driver package needs: a directory under `src/Drivers`, an entry in the
