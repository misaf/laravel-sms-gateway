...
test('can send request through smsir driver', function (): void {
    config()->set('laravel-sms-gateway.default', 'smsir');
    config()->set('laravel-sms-gateway-smsir.api_key', 'smsir-api-key');
...
test('prefers the base URL configured in the driver config over the driver default', function (): void {
    config()->set('laravel-sms-gateway.default', 'smsir');
    config()->set('laravel-sms-gateway-smsir.base_url', 'https://services-override.example.test/v1/');
