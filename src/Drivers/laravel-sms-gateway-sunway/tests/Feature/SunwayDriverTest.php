...
test('sunway driver sends credentials as query parameters', function (): void {
    ...
    config()->set('laravel-sms-gateway-sunway.username', 'username');
    config()->set('laravel-sms-gateway-sunway.password', 'password');
...
test('prefers the base URL configured in the driver config over the driver default', function (): void {
    config()->set('laravel-sms-gateway.default', 'sunway');
    config()->set('laravel-sms-gateway-sunway.base_url', 'https://services-override.example.test/smsws/');
