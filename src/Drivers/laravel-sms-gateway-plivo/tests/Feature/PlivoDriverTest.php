...
test('can send SMS via Plivo driver', function (): void {
    config()->set('laravel-sms-gateway.default', 'plivo');
    config()->set('laravel-sms-gateway-plivo.auth_id', 'MA123');
    config()->set('laravel-sms-gateway-plivo.auth_token', 'plivo-auth-token');
...
test('prefers the base URL configured in the driver config over the driver default', function (): void {
    config()->set('laravel-sms-gateway.default', 'plivo');
    config()->set('laravel-sms-gateway-plivo.base_url', 'https://services-override.example.test/v1/Account/MA123/');
